...
function localDateTimeInput(value: string | null) { if (!value) return ''; const date = new Date(value); const local = new Date(date.getTime() - date.getTimezoneOffset() * 60000); return local.toISOString().slice(0, 16); }
...
export function ContentManager() {
  ...
  async function load() {
    try {
      ...
      const normalized = data.map((entry: ContentItem) => ({ ...blank, ...entry })); setItems(normalized); const requested = new URLSearchParams(window.location.search).get('edit'); if (requested) { const match = normalized.find((entry: ContentItem) => entry.id === requested); if (match) setItem(match); } setMessage(`${data.length} content items loaded.`);
  ...
  return <div className="admin-layout">
    <form className="card form-grid sticky-editor" onSubmit={save}>
      ...
      <label>Schedule for<input className="input" type="datetime-local" value={localDateTimeInput(item.scheduled_for)} onChange={(e) => setItem({ ...item, scheduled_for: e.target.value ? new Date(e.target.value).toISOString() : null })} /><small>Shown in your local timezone. This records publishing intent; it does not auto-post to external channels.</small></label>
