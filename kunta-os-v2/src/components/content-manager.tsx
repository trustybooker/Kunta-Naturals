...
export function ContentManager() {
  ...
  return <div className="admin-layout">
    ...
    <section className="grid product-list">{items.length === 0 && <div className="card empty-state"><h2>No content yet</h2><p>Create a draft, send it through review, then publish only after approval.</p></div>}{items.map((entry) => <article className="card product-row" key={entry.id}><div className="row-between"><span className="badge">{entry.status}</span><span>{entry.channel}</span></div><h2>{entry.title}</h2><p>{entry.body.slice(0, 180)}{entry.body.length > 180 ? '…' : ''}</p>{entry.scheduled_for && <p><strong>Planned:</strong> {new Date(entry.scheduled_for).toLocaleString()}</p>}<button type="button" className="button-secondary" onClick={() => { setItem(entry); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>Edit</button></article>)}</section>
  </div>;
}
