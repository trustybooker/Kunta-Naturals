...
export function AnalyticsDashboard() {
  ...
  useEffect(() => { fetch('/api/admin/analytics', { cache: 'no-store' }).then(async (response) => { if (response.status === 401 || response.status === 403) { window.location.href = '/login?next=%2Fanalytics'; return; } const body = await response.json(); if (!response.ok) throw new Error(body.error); setData(body); }).catch(() => setError('Analytics could not be loaded. Check your connection and try again.')); }, []);
