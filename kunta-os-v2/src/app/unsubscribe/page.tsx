...
export default async function UnsubscribePage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  return <main className="shell"><section className="card" style={{ maxWidth: 680, margin: '4rem auto', padding: '2.5rem' }}><p className="eyebrow">Email preferences</p><h1>{email ? 'You’re unsubscribed.' : 'This link is not valid.'}</h1><p>{email ? 'Kunta Naturals will stop sending marketing and course emails to this address. Transactional order and delivery messages may still be sent when required.' : 'Open the unsubscribe link from the most recent Kunta Naturals email or contact support.'}</p><a className="button inline-button" href="https://kuntanaturals.com">Return to Kunta Naturals</a></section></main>;
}
